...
class MetricsCollector:
    ...
    def __init__(
        self,
        otel_endpoint: str,
        service_name: str = "redis-load-test",
        ...
        self.instance_id = (
            ...
            else f"{app_name}-{str(uuid.uuid4())[:8]}"
        )
    ...
    def _setup_opentelemetry(self):
        ...
        try:
            ...
            self.logger.info(
                f"OpenTelemetry setup completed with endpoint: {self.otel_endpoint}"
            )
    ...
    def record_operation(
        self, operation: str, duration: float, success: bool, error_type: str = None
    ):
    ...
    def record_client_init_duration(
        self, duration: float, client: str = "standalone-sync"
    ):
    ...
    def get_overall_stats(self) -> Dict:
        ...
        with self._lock:
            ...
            stats = {
                ...
                "overall_throughput": total_ops / total_duration
                if total_duration > 0
                else 0,
                "overall_success_rate": total_success / total_ops
                if total_ops > 0
                else 0,
            }
    ...
    def get_final_test_summary(self) -> "Statistics":
        ...
        summary = Statistics(
            self.app_name, self.instance_id, self.run_id, self.version, workload_name
        )
        ...
        summary.avg_reconnection_duration_ms = (
            0  # TODO: Implement reconnection duration tracking
        )
    ...
    def print_summary(self, clients=None, threads_per_client=None):
        """Print final test summary in standardized format and log to file."""
        summary = self.get_final_test_summary()

        # Build summary lines
        summary_lines = []
        summary_lines.append("=" * 60)
        summary_lines.append("FINAL TEST SUMMARY")
        summary_lines.append("=" * 60)
        if clients is not None:
            summary_lines.append(f"Clients: {clients}")
        if threads_per_client is not None:
            summary_lines.append(f"Threads per Client: {threads_per_client}")
        summary_lines.append(f"Workload Name: {summary.workload_name}")
        summary_lines.append(
            f"Total test run time: {summary.run_end - summary.run_start:.2f}s"
        )
        summary_lines.append(f"Total Commands: {summary.total_commands_count:,}")
        summary_lines.append(
            f"Successful Commands: {summary.successful_commands_count:,}"
        )
        summary_lines.append(f"Failed Commands: {summary.failed_commands_count:,}")
        success_rate = (
            (summary.successful_commands_count / summary.total_commands_count)
            if summary.total_commands_count > 0
            else 0.0
        )
        summary_lines.append(f"Success Rate: {success_rate:.2%}")
        summary_lines.append(
            f"Overall Throughput: {summary.overall_throughput:,} ops/sec"
        )
        if summary.reconnection_count > 0:
            summary_lines.append(
                f"Avg Reconnection Duration: {summary.avg_reconnection_duration_ms:.1f}ms"
            )
        summary_lines.append("=" * 60)

...
class RedisTestLogger:
    ...
    def log_operation_result(self, operation: str, success: bool, duration: float, error: str = None):
        """Log operation result with standardized format."""
        status = "SUCCESS" if success else "FAILED"
        message = f"Operation: {operation} | Status: {status} | Duration: {duration:.4f}s"
...
def log_error_with_traceback(message: str, exception: Exception = None):
    ...
    if _logger_instance:
        ...
        logging.error(f"{message}: {str(exception) if exception else ''}", exc_info=True)
